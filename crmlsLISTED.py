"""
This pythong script is for concatenates all monthly CRMLSListing CSV files into one combined
...
LISTING_PATTERN = os.path.join(DATA_FOLDER, "CRMLSListing*.csv")
OUTPUT_FILE = os.path.join(DATA_FOLDER, "listings.csv")
 
 
def main():
    files = sorted(glob.glob(LISTING_PATTERN))
 
    if not files:
        raise FileNotFoundError(
            f"No files found matching {LISTING_PATTERN}. Check DATA_FOLDER and file naming."
        )
 
    print(f"Found {len(files)} monthly listing files:")
    for f in files:
